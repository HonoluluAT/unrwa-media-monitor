...
{sorted.slice(0,10).sort((a:any,b:any)=>new Date(b.date).getTime()-new Date(a.date).getTime()).map((a:any,i:number)=><Row key={a.id||i} a={a} rank={i+1}/>)}            {articles.length===0&&<div style={{padding:40,textAlign:"center",color:"var(--t3)",fontSize:13}}>No articles yet. Click "Update" to run your first scan.</div>}
          </div>
